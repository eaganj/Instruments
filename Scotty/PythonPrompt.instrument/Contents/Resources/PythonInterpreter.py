...
class StringReprPrinter(object):
    def __init__(self, obj, rep=None):
        self._obj = obj
        self._repr = rep
...
class AsyncInteractiveConsole(InteractiveConsole):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.locals.update(sys.modules['Foundation'].__dict__)
        self.locals.update(sys.modules['AppKit'].__dict__)
        self.locals['objc'] = objc
        self.locals['help'] = StringReprPrinter(pydoc.help, 
                                'Type help() for interactive help, or help(object) for help about object.')
        self.locals['copyright'] = StringReprPrinter(sys.copyright)
        self.locals['credits'] = StringReprPrinter(u"""\
        Thanks to CWI, CNRI, BeOpen.com, Zope Corporation and a cast of thousands
        for supporting Python development.  See www.python.org for more information.
        """)
        
        # licenseFile = os.path.normpath(os.path.join(os.path.dirname(site.__file__), 'LICENSE.txt'))
        # with open(licenseFile, 'r') as f:
        #     licenseText = f.read()
        # self.locals['license'] = StringReprPrinter(licenseText)
    ...
    def asyncinteract(self, write=None, banner=None):
        ...
        if write is None:
            write = self.write
        cprt = u'Type "help", "copyright", or "credits" for more information.\n' \
               u'\n' \
               u'>>> from Foundation import *\n' \
               u'>>> from AppKit import *\n' \
               u'>>> import objc'
        if banner is None:
...
class PythonInterpreterController(NSWindowController):
    # def init(self):
    #         d = {}
    #         
